...
class MatchARNet(utils.backbone.VGG16_bn):
    ...
    def update_queries(self, Q, n_points_sample, in_training, perm_mats= None, sampled_matches=None):
        ...
        for batch_idx in range(B):
            ...
            if in_training:
                ...
                # perm_mats_masked = perm_mats[0][batch_idx] * perm_mats_mask[batch_idx]
                # perm_mats_masked = perm_mats_masked.to(perm_mats[0].device)
                idx = sampled_matches[batch_idx]
                # idx = torch.nonzero(perm_mats_masked == 1)
            else:
    ...
    def forward(
        ...
        queries = queries.view(B, N_s * N_t, -1)
        """
        queries = self.update_queries(queries, n_points_sample, in_training, perm_mats=perm_mats, sampled_matches=sampled_matches)
